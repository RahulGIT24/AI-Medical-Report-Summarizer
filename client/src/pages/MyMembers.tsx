              ...
              {filteredPatients.map((patient, index) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-bold text-lg shadow-inner border ${
                        patient.gender === "M" ? "bg-blue-500/10 text-blue-400 border-blue-500/20" : "bg-pink-500/10 text-pink-400 border-pink-500/20"
                      }`}
                    >
                  ...
                  <ChevronRight
                    ...
                    className={`transition-transform duration-300 ${
                      selectedPatient?.id === patient.id ? "text-blue-400 translate-x-1" : "text-gray-600 group-hover:text-gray-400"
                    }`}
                  />
                </motion.button>
              ))}
        ...
        <AnimatePresence mode="wait">
          {selectedPatient ? (
            ...
            >
                ...
                <div className="flex items-center gap-6">
                  <motion.div 
                    ...
                    className={`w-20 h-20 rounded-4xl flex items-center justify-center font-bold text-3xl shadow-xl border ${
                      selectedPatient.gender === "M" ? "bg-blue-500/10 text-blue-400 border-blue-500/20" : "bg-pink-500/10 text-pink-400 border-pink-500/20"
                    }`}
                  >
                    {selectedPatient.first_name[0]}{selectedPatient.last_name[0]}
                  </motion.div>
                  <div>
                    <motion.h1 initial={{ y: 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="text-3xl md:text-4xl font-bold text-white mb-2 tracking-tight">
                      {selectedPatient.first_name} {selectedPatient.last_name}
                    </motion.h1>
                    <motion.p initial={{ y: 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.1 }} className="text-sm text-gray-400 font-medium bg-gray-800/50 w-fit px-3 py-1 rounded-full border border-gray-700/50">
                ...
                <div className="flex gap-8 mt-10 border-b border-gray-800/80">
                  {([
                    ...
                    >
                      ...
                      {tab.label}
                      {activeTab === tab.id && (
                        <motion.div layoutId="mainTabIndicator" className="absolute bottom-0 left-0 w-full h-0.5 bg-blue-500 rounded-t-full shadow-[0_-2px_8px_rgba(59,130,246,0.5)]" />
                      )}
                    </button>
                  ))}
                </div>
              ...
              <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
                <AnimatePresence mode="wait">
                  ...
                  {activeTab === "upload" && (
                    <motion.div key="upload-tab" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="max-w-4xl mx-auto w-full">
                        ...
                        {(["upload", "view"] as const).map((subTab) => (
                          <button
                            ...
                            className={`relative flex items-center gap-2 px-6 py-2.5 rounded-lg text-sm font-medium transition-colors z-10 ${
                              uploadSubTab === subTab ? "text-white" : "text-gray-400 hover:text-gray-200"
                            }`}
                          >
                            {uploadSubTab === subTab && (
                              <motion.div layoutId="subTabIndicator" className="absolute inset-0 bg-gray-800 rounded-lg border border-gray-700 shadow-sm z-[-1]" />
                            )}
                            {subTab === "upload" ? <UploadCloud size={18} /> : <List size={18} />}
                            {subTab === "upload" ? "Upload New" : "View Reports"}
                          </button>
                        ))}
                      </div>
                      ...
                      <AnimatePresence mode="wait">
                        {uploadSubTab === "upload" ? (
                          <motion.div key="upload-content" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
                            <input type="file" multiple className="hidden" ref={fileInputRef} onChange={handleFileSelect} accept=".pdf,.jpg,.jpeg,.png,.doc,.docx" />
                            ...
                            >
                              ...
                              <h3 className="text-2xl font-bold text-white mb-3 tracking-tight">Upload Medical Records</h3>
                              <p className="text-gray-400 mb-8 max-w-sm mx-auto">
                                Click or drop PDFs and images here to securely add them to {selectedPatient.first_name}'s encrypted profile.
                              </p>
                            ...
                            <AnimatePresence>
                              {selectedFiles.length > 0 && (
                                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="mt-8 overflow-hidden">
                                  <h4 className="text-lg font-bold text-white mb-4">Selected Files ({selectedFiles.length}/5)</h4>
                                  <div className="space-y-3 mb-8">
                                    {selectedFiles.map((file, index) => (
                                      ...
                                      >
                                        ...
                                        <button onClick={(e) => { e.stopPropagation(); removeFile(index); }} className="text-gray-500 hover:text-red-400 hover:bg-red-400/10 transition-colors p-2 rounded-lg">
                                          <X size={18} />
                                        </button>
                                      </motion.div>
                                    ))}
                                  ...
                                  >
                                    {isUploading ? (
                                      <><Loader2 className="animate-spin" size={22} /> Uploading securely...</>
                                    ) : (
                                      <><UploadCloud size={22} /> Confirm & Upload {selectedFiles.length} Files</>
                                    )}
                                  </motion.button>
                                </motion.div>
                              )}
                          ...
                          <motion.div key="view-content" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
                            {loadingReports ? (
                  ...
                  {/* TRENDS TAB */}
                  {activeTab === "trends" && (
                    <motion.div key="trends-tab" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="h-full">
            ...
            >
              ...
              <h2 className="text-3xl font-bold text-white mb-3 tracking-tight">No Member Selected</h2>
              <p className="text-gray-400 max-w-md text-lg leading-relaxed">
                Select a family member from the sidebar to securely view their reports, analyze health trends, and consult the AI.
              </p>
            </motion.div>
          )}
        </AnimatePresence>
