...
const ReportDetailPage = () => {
    ...
    const [expandedSections, setExpandedSections] = useState<Set<string>>(new Set(['metadata', 'test-results', 'summary']));
    ...
    const handleSummarize = async () => {
        try {
            setSummarizing(true)
            const response = await apiCall(`/report/summarise/${id}`);
    ...
    const fetchSummary = async () => {
        try {
            setLoading(true);
            const response = await apiCall(`/report/aisummary/${id}`);
    ...
    const getOutcomeColor = (outcome?: string, isAbnormal?: boolean, isCritical?: boolean) => {
        ...
        if (isAbnormal) return 'text-yellow-400 bg-yellow-500/10 border-yellow-500/50';
        if (outcome?.toLowerCase().includes('positive')) return 'text-red-400 bg-red-500/10 border-red-500/50';
        if (outcome?.toLowerCase().includes('negative')) return 'text-green-400 bg-green-500/10 border-green-500/50';
                        ...
                        <div className="flex items-center space-x-4">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-gray-400">
                                    {report.report_metadata?.report_type || 'Health Report'} • {formatDate(report.created_at)}
                                </p>
                            </div>
                        </div>
                        <button
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
                ...
                {report.error && (
                    <div className="bg-red-500/10 border border-red-500/50 rounded-2xl p-4 flex items-start space-x-3">
                        ...
                        <div>
                            <h3 className="text-red-400 font-semibold mb-1">Error Processing Report</h3>
                ...
                {report.reports_media && report.reports_media.length > 0 && (
                    <Section title="AI Report Summary" icon={FileText} sectionKey="summary">
                        <div className="flex flex-col gap-3">
                            <p className="text-gray-300">{generatedSummary ? "AI Generated summary of report" : "Summarize Report using AI"}</p>
                            {
                            ...
                            {
                                summarizing && (
                                    <div className="flex justify-center items-center"> <p className="px-4 py-2 rounded-xl bg-gray-800/70 border border-gray-700 
                                    text-white font-medium backdrop-blur-sm hover:bg-gray-700 
                                    active:scale-95 transition-all duration-150">Summarizing Report Please Wait.......</p></div>
                                )
                            }
                            {
                                generatedSummary && (
                    ...
                    <Section title="Patient & Report Information" icon={User} sectionKey="metadata">
                        ...
                        {report.report_metadata.notes && (
                            <div className="mt-4 bg-gray-900/50 rounded-lg p-4">
                                ...
                                <p className="text-white leading-relaxed">{report.report_metadata.notes}</p>
                            </div>
                        )}
                    </Section>
                )}
                            ...
                            <table className="w-full">
                                <thead>
                                    <tr className="border-b border-gray-700">
                                        <th className="text-left py-3 px-4 text-gray-400 font-semibold text-sm">Test Name</th>
                                        <th className="text-left py-3 px-4 text-gray-400 font-semibold text-sm">Result</th>
                                        <th className="text-left py-3 px-4 text-gray-400 font-semibold text-sm">Reference Range</th>
                                    ...
                                    {report.test_results.map((test, index) => (
                                        <tr key={index} className="border-b border-gray-800 hover:bg-gray-900/50">
                                            <td className="py-4 px-4">
                                                ...
                                                {test.test_category && (
                                                    <p className="text-gray-500 text-sm">{test.test_category}</p>
                                                )}
                                            </td>
                                            <td className="py-4 px-4">
                                                <p className="text-white">
                                                    {test.result_value} {test.unit && <span className="text-gray-400">{test.unit}</span>}
                                                </p>
                                            </td>
                                            ...
                                            <td className="py-4 px-4">
                                                {test.outcome && (
                                                    <span className={`inline-flex items-center space-x-1 px-3 py-1 rounded-lg border text-sm font-medium ${getOutcomeColor(test.outcome, test.is_abnormal, test.is_critical)}`}>
                                                        {test.is_critical && <AlertCircle size={14} />}
                                                        {test.is_abnormal && !test.is_critical && <AlertCircle size={14} />}
                                                        {!test.is_abnormal && !test.is_critical && <CheckCircle2 size={14} />}
                ...
                {report.specimen_validity && (
                    <Section title="Specimen Validity" icon={CheckCircle2} sectionKey="specimen">
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            ...
                            {report.specimen_validity.specific_gravity !== undefined && (
                                <div className="bg-gray-900/50 rounded-lg p-4">
                                    ...
                                    <p className="text-white font-semibold">{report.specimen_validity.specific_gravity}</p>
                                    {report.specimen_validity.specific_gravity_status && (
                                        <p className="text-gray-500 text-sm">{report.specimen_validity.specific_gravity_status}</p>
                                    )}
                                </div>
                            )}
                            {report.specimen_validity.ph_level !== undefined && (
                                <div className="bg-gray-900/50 rounded-lg p-4">
                                    ...
                                    <p className="text-white font-semibold">{report.specimen_validity.ph_level}</p>
                                    {report.specimen_validity.ph_status && (
                                        <p className="text-gray-500 text-sm">{report.specimen_validity.ph_status}</p>
                                    )}
                                </div>
                            )}
                            {report.specimen_validity.creatinine !== undefined && (
                                <div className="bg-gray-900/50 rounded-lg p-4">
                                    ...
                                    <p className="text-white font-semibold">{report.specimen_validity.creatinine}</p>
                                    {report.specimen_validity.creatinine_status && (
                                        <p className="text-gray-500 text-sm">{report.specimen_validity.creatinine_status}</p>
                                    )}
                                </div>
                            )}
                            {report.specimen_validity.oxidants && (
                                <div className="bg-gray-900/50 rounded-lg p-4">
                                    ...
                                    <p className="text-white font-semibold">{report.specimen_validity.oxidants}</p>
                                    {report.specimen_validity.oxidants_status && (
                                        <p className="text-gray-500 text-sm">{report.specimen_validity.oxidants_status}</p>
                                    )}
                                </div>
                            )}
                        </div>
                            ...
                            {report.screening_tests.map((test, index) => (
                                <div key={index} className="bg-gray-900/50 rounded-lg p-4 flex items-center justify-between">
                                    <div>
                                        <p className="text-white font-medium">{test.test_name}</p>
                                        {test.result_value && (
                                            <p className="text-gray-400 text-sm">Value: {test.result_value}</p>
                                        )}
                                        {test.cutoff_value && (
                                            <p className="text-gray-500 text-sm">Cutoff: {test.cutoff_value}</p>
                                        )}
                                    </div>
                                    {test.outcome && (
                                        <span className={`px-3 py-1 rounded-lg border text-sm font-medium ${getOutcomeColor(test.outcome)}`}>
                                            {test.outcome}
                                        </span>
                                    )}
                                </div>
                ...
                {report.confirmation_tests && report.confirmation_tests.length > 0 && (
                    <Section title="Confirmation Tests" icon={Beaker} sectionKey="confirmation">
                        <div className="space-y-3">
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                                        ...
                                        {test.result_value && (
                                            <div>
                                                ...
                                                <p className="text-gray-300">{test.result_value} {test.unit}</p>
                                            </div>
                                        )}
                                        {test.cutoff_value && (
                                            <div>
                ...
                {report.medications && report.medications.length > 0 && (
                    <Section title="Reported Medications" icon={Pill} sectionKey="medications">
                            ...
                            {report.medications.map((med, index) => (
                                <div key={index} className="bg-gray-900/50 rounded-lg p-3 flex items-center justify-between">
                                    ...
                                    {med.is_tested !== undefined && (
                                        <span className={`px-2 py-1 rounded text-xs ${med.is_tested ? 'bg-blue-500/20 text-blue-400' : 'bg-gray-500/20 text-gray-400'}`}>
                    ...
                    >
                        <div className="sticky top-0 bg-gray-800 border-b border-gray-700 p-4 flex items-center justify-between z-10">
                            <div>
                                ...
                                <p className="text-sm text-gray-400">
                                    Viewing {selectedMediaIndex + 1} of {report.reports_media.length}
                                </p>
                            </div>
                            <button
                                onClick={() => setShowMediaModal(false)}
